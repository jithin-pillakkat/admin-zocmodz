...
class Bike_model extends CI_Model
{
    ...
    public function getBikes($search, $order, $order_column, $order_dir, $start, $length, $draw)
    {

        $column = ["id", "title", "image", "bike_status",  "created_at"];
        ...
        foreach ($result as $row) {  // preparing an array
            $nestedData = array();
            $status = ($row->bike_status == 1)? 'checked' : '';
            $nestedData[] = $i++;
